...
{#key message.id}
	<div class=" flex w-full message-{message.id}" id="message-{message.id}">
		<ProfileImage
			src={modelfiles[message.model]?.imageUrl ?? `${WEBUI_BASE_URL}/static/favicon.ico`}
		/>
									...
									{#if !message.regenerateAttempted}
									  ...
									  <div class="flex mt-2 mb-4 space-x-2 border px-4 py-3 border-red-800 bg-red-800/30 font-medium rounded-lg">
										...
										<Tooltip content="Regenerate" placement="bottom">
										  <button
											type="button"
											class="p-1 rounded dark:hover:text-white hover:text-black transition regenerate-response-button"
											on:click={regenerateResponse}
										  >
											<!-- Add your regenerate button icon or text here -->
										  </button>
										</Tooltip>
									  </div>
									{/if}
